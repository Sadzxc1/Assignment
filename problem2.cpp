...
int main()
{
    //using mthod 1 of implementation//
    int n = 0;
    ProgrammingProblem1A pp1a{};
    pp1a.timeToProcess(9);
    double time1A = 0, time2B = 0;
    for (n = 0; time1A <= time2B; n++)
    {
        time1A = FindTime(pp1a, n);
        time2B = FindTime(ProgrammingProblem2B{}, n);
        if (time1A > time2B)
            break;
    }
    std::cout << "Using Timed processor\n";
    std::cout << "Loop 2B is faster than Loop 1A at n= " << n << std::endl;
    std::cout << "Time consumed by loop 2B is    (n = " << n << "):  " << time2B << std::endl;
